...
function renderAuth(){
  ...
  if (firebaseEnabled && auth && auth.currentUser) {
    ...
    area.innerHTML = `<div>Signed in: ${u.email || u.displayName} <button id="btn-signout">Sign out</button></div>`;
    ...
    const btn = document.createElement('button'); btn.textContent='Add local user'; btn.onclick=()=>{ ensureLocalUser(prompt('username:')||('user_'+Math.random().toString(36).slice(2,5))); renderAll(); }; area.appendChild(btn);
  }
}
